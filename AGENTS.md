...
- Primary LiveView surfaces live under `lib/toukon_ai_shogi_web/live/`; `LobbyLive` manages matchmaking + presence, `GameRoomLive` streams shared state, and `BoardComponents` render the board
- Multiplayer state lives in `ToukonAiShogi.Matchmaking` (queue) and `ToukonAiShogi.GameRooms` (per-room GenServer + PubSub broadcast)
